...
class doFit_wj_and_wlvj:
    ...
    def get_WJets_mlvj_correction_sb_lo_to_signal_region(self,label, mlvj_model):#exo-vv method: extract M_lvj shape of signal_region from sb_lo
        ...
        if label=="_WJets0":
            ...
            self.draw_canvas_with_pull( mplot_sb_lo, mplot_pull_sideband,parameters_list,"plots_%s_%s/other/"%(self.channel,self.PS_model), "m_lvj%s_sb_lo_sim"%(label),"",1,0)
            ...
            self.draw_canvas_with_pull( mplot_signal_region, mplot_pull_signal_region,parameters_list,"plots_%s_%s/other/"%(self.channel,self.PS_model), "m_lvj%s_signal_region_sim"%(label),"",1,0);
        ...
        #leg=self.legend4Plot(mplot,0,0,-0.07 , 0, 0.22);mplot.addObject(leg);
        ...
        self.draw_canvas(mplot,"plots_%s_%s/other/"%(self.channel,self.PS_model),"correction_pdf%s_%s_%s_M_lvj_signal_region_to_sideband"%(label,self.PS_model,self.MODEL_4_mlvj),0,0);
    ...
    def fit_mj_single_MC(self,in_file_name, label, in_model_name, additioninformation=""):
        ...
        self.draw_canvas_with_pull( mplot, mplot_pull,parameters_list,"plots_%s_%s/m_j_fitting%s_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, additioninformation, self.wtagger_label, self.nPV_min, self.nPV_max), label+in_file_name, in_model_name)
        rfresult.Print(); 
        #rfresult.covarianceMatrix().Print(); #raw_input("ENTER"); 
    ...
    def fit_mj_singlebackground_MC_TTbar_controlsample(self,in_file_name, label, in_model_name):
        ...
        parameters_list=model.getParameters(rdataset_mj);
        mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.1);
        self.draw_canvas_with_pull( mplot, mplot_pull,parameters_list,"plots_%s_%s/m_j_fitting_TTbar_controlsample_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, self.wtagger_label, self.nPV_min, self.nPV_max), label+in_file_name, in_model_name)
        rfresult.Print();
    ...
    #    model_pdf_TTbar.plotOn(mplot,RooFit.Normalization(scale_number_TTbar),RooFit.Name("TTbar"), RooFit.AddTo("STop_invisible"),RooFit.DrawOption("F"), RooFit.FillColor(self.color_palet["TTbar"]), RooFit.LineColor(kBlack), RooFit.VLines())
    #    model_pdf_STop.plotOn(mplot,RooFit.Normalization(scale_number_STop),RooFit.Name("STop"), RooFit.AddTo("VV_invisible"), RooFit.DrawOption("F"), RooFit.FillColor(self.color_palet["STop"]), RooFit.LineColor(kBlack), RooFit.VLines())
    #    model_pdf_VV.plotOn(mplot,RooFit.Normalization(scale_number_VV),RooFit.Name("VV"), RooFit.AddTo("WJets_invisible"),RooFit.DrawOption("F"), RooFit.FillColor(self.color_palet["VV"]), RooFit.LineColor(kBlack), RooFit.VLines())
    #    model_pdf_WJets.plotOn(mplot,RooFit.Normalization(scale_number_WJets),RooFit.Name("WJets"),RooFit.DrawOption("F"), RooFit.FillColor(self.color_palet["WJets"]), RooFit.LineColor(kBlack), RooFit.VLines())
    ...
    #     
    #    mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.1);
    #    self.draw_canvas_with_pull( mplot, mplot_pull,parameters_list,"plots_%s_%s/m_j_fitting_TTbar_controlsample_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, self.wtagger_label, self.nPV_min, self.nPV_max), in_file_name, "ErfExp2Gaus"+"Total"+self.with_tau2tau1)
    #    self.draw_canvas(mplot,"plots_%s_%s/m_j_fitting_TTbar_controlsample_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, self.wtagger_label, self.nPV_min, self.nPV_max),"control_%s_%s_%s"%(self.wtagger_label,self.channel,self.with_tau2tau1));
    #    
    ...
    def ScaleFactor_forPureWJet_TTbar_controlsample(self,in_file_name, fit_or_not=1):#stateoftau2tau1cut="", or "failtau2tau1cut_"
        ...
        self.draw_canvas(xframe_data,"plots_%s_%s/m_j_fitting_TTbar_controlsample_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, self.wtagger_label, self.nPV_min, self.nPV_max),"control_%s_%s"%(self.wtagger_label,self.channel));
        self.draw_canvas(xframe_data_fail,"plots_%s_%s/m_j_fitting_TTbar_controlsample_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, self.wtagger_label, self.nPV_min, self.nPV_max),"control_%s_%s_fail"%(self.wtagger_label,self.channel));
    ...
    def fit_mlvj_model_single_MC(self,in_file_name, label, in_range, mlvj_model, deco=0, show_constant_parameter=0, logy=0):# model = shape + normalization
        ...
        if deco :
            ...
            if label=="_TTbar" and in_range=="_signal_region": 
                ...
                model_pdf_deco.plotOn(mplot_deco,RooFit.Name("t#bar{t}_Powheg"),RooFit.LineColor(kBlack));
                ...
                #self.workspace4fit_.pdf("model_TTbar_Powheg_signal_region_%s_mlvj"%(self.channel)).plotOn(mplot_deco,RooFit.Name("t#bar{t}_Powheg"), RooFit.LineStyle(kDashed),RooFit.LineColor(kBlack));
                self.workspace4fit_.pdf("model_TTbar_MG_signal_region_%s_mlvj"%(self.channel)).plotOn(mplot_deco,RooFit.Name("t#bar{t}_MG"), RooFit.LineStyle(kDashed),RooFit.LineColor(kBlack));
                self.workspace4fit_.pdf("model_TTbar_scaleUp_signal_region_%s_mlvj"%(self.channel)).plotOn(mplot_deco,RooFit.Name("t#bar{t}_scaleUp"), RooFit.LineColor(kRed));
                self.workspace4fit_.pdf("model_TTbar_scaleDn_signal_region_%s_mlvj"%(self.channel)).plotOn(mplot_deco,RooFit.Name("t#bar{t}_scaleDn"), RooFit.LineStyle(kDashed),RooFit.LineColor(kRed));
                self.workspace4fit_.pdf("model_TTbar_matchUp_signal_region_%s_mlvj"%(self.channel)).plotOn(mplot_deco,RooFit.Name("t#bar{t}_matchUp"), RooFit.LineColor(kBlue));
                self.workspace4fit_.pdf("model_TTbar_matchDn_signal_region_%s_mlvj"%(self.channel)).plotOn(mplot_deco,RooFit.Name("t#bar{t}_matchDn"), RooFit.LineStyle(kDashed),RooFit.LineColor(kBlue));
        ...
        mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.1);
        self.draw_canvas_with_pull( mplot, mplot_pull,parameters_list,"plots_%s_%s/m_lvj_fitting/"%(self.channel,self.PS_model), in_file_name,"m_lvj"+in_range+mlvj_model, show_constant_parameter, logy)
    ...
    def fit_WJetsNormalization_in_Mj_signal_region(self,label,massscale=""): # to  get the normalization of WJets in signal_region
        ...
        wjets_norm_error=0.10;
        if self.channel=="mu": wjets_norm_error=0.056
        rrv_number_data_mj.setError(TMath.Sqrt(
                ( self.workspace4fit_.var("rrv_number_TTbar%s_%s_mj"%(massscale,self.channel)).getError() )**2+
                ( self.workspace4fit_.var("rrv_number_TTbar%s_%s_mj"%(massscale,self.channel)).getVal()*( self.lumi_uncertainty**2+ self.rrv_wtagger_eff_reweight_forT_relative_error**2 )**0.5   )**2+
                ( self.workspace4fit_.var("rrv_number_STop%s_%s_mj"%(massscale,self.channel)).getError() )**2+
                ( self.workspace4fit_.var("rrv_number_STop%s_%s_mj"%(massscale,self.channel)).getVal()*self.XS_STop_uncertainty )**2+
                ( self.workspace4fit_.var("rrv_number_VV%s_%s_mj"%(massscale,self.channel)).getError() )**2+
                ( self.workspace4fit_.var("rrv_number_VV%s_%s_mj"%(massscale,self.channel)).getVal()*self.XS_VV_uncertainty )**2+
                #( self.workspace4fit_.var("rrv_number%s_%s_mj"%(label,self.channel)).getError() )**2
                ( self.workspace4fit_.var("rrv_number%s_%s_mj"%(label,self.channel)).getVal()*wjets_norm_error )**2
                ));
        #rrv_number_data_mj.Print(); raw_input("ENTER");
        ...
        if label=="_WJets0":
            ...
            rdataset_data_mj.plotOn(mplot, RooFit.Name("data"), RooFit.MarkerSize(1.5), RooFit.DataError(RooAbsData.SumW2), RooFit.XErrorSize(0) );
            graph_error = draw_error_band(rdataset_data_mj, model_data, rrv_number_data_mj,rfresult,mplot,self.color_palet["Uncertainty"],"F");
            rdataset_data_mj.plotOn(mplot, RooFit.Name("data_invisible"), RooFit.MarkerSize(1.5), RooFit.DataError(RooAbsData.SumW2), RooFit.XErrorSize(0) );
            mplot_pull=self.get_pull(rrv_mass_j,mplot, graph_error);
            #signal window
            lowerLine = TLine(self.mj_signal_min,0.,self.mj_signal_min,mplot.GetMaximum()); lowerLine.SetLineWidth(2); lowerLine.SetLineColor(kGray+2); lowerLine.SetLineStyle(9);
            upperLine = TLine(self.mj_signal_max,0.,self.mj_signal_max,mplot.GetMaximum()); upperLine.SetLineWidth(2); upperLine.SetLineColor(kGray+2); upperLine.SetLineStyle(9);
            ...
            parameters_list=model_data.getParameters(rdataset_data_mj);
            mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.1);
            self.draw_canvas_with_pull( mplot, mplot_pull,parameters_list,"plots_%s_%s/m_j_fitting_wtaggercut%s_nPV%sto%s/"%(self.channel,self.PS_model, self.wtagger_label, self.nPV_min, self.nPV_max), "m_j_sideband%s_%s"%(label,self.channel),"",1)
   ...
   ######## ++++++++++++++
    def fit_mlvj_in_Mj_sideband(self, label, mlvj_region, mlvj_model): 
        ...
        if label=="_WJets0":
            ...
            mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.1)
            self.draw_canvas_with_pull( mplot, mplot_pull,parameters_list,"plots_%s_%s/m_lvj_fitting/"%(self.channel,self.PS_model), "m_lvj_sb_lo%s_%s_%s"%(label,self.channel,self.higgs_sample),"",1,1)
    ...
    def prepare_limit(self,mode):
        ...
        if self.higgs_sample=="ggH600" or self.higgs_sample=="ggH700":shape_para_error_alpha=1.4;
    ...
    def read_workspace(self):
        ...
        #model_pdf_ggH.plotOn(mplot,RooFit.Normalization(scale_number_ggH*signal_scale),RooFit.Name("%s #times %s"%(self.higgs_sample, signal_scale)),RooFit.DrawOption("L"), RooFit.LineColor(self.color_palet["ggH"]), RooFit.LineStyle(2), RooFit.VLines());
        #model_pdf_vbfH.plotOn(mplot,RooFit.Normalization(scale_number_vbfH*signal_scale),RooFit.Name("%s #times %s"%(self.vbfhiggs_sample,signal_scale)),RooFit.DrawOption("L"), RooFit.LineColor(self.color_palet["vbfH"]), RooFit.LineStyle( 9), RooFit.VLines());
        if self.higgs_sample=="ggH600":
    ...
    ######## ++++++++++++++
    def get_pull(self, rrv_x, mplot_orig, graph_error_of_MC=0): #graph_error_of_MC is TGraphAsymmErrors
        hpull=mplot_orig.pullHist();
        for i in range(hpull.GetN() ): 
            hpull.SetPoint( i, 800, 2)
        ...
        mplot_pull.GetYaxis().SetTitleSize(0.11);
        mplot_pull.GetYaxis().SetTitleOffset(0.30);
        return mplot_pull;
    ...
    def draw_canvas_with_pull(self, mplot, mplot_pull,parameters_list,in_directory, in_file_name, in_model_name="", show_constant_parameter=0, logy=0):# mplot + pull + parameters
        ...
        mplot.GetXaxis().SetTitleOffset(0.95);
        mplot.GetXaxis().SetTitleSize(0.04);
        mplot.GetXaxis().SetLabelSize(0.03);

        mplot.GetYaxis().SetTitleOffset(1.35);
        mplot.GetYaxis().SetTitleSize(0.035);
    ...
    def fit_Signal(self):
        ...
        self.fit_mj_single_MC(self.file_ggH,"_%s"%(self.higgs_sample),"2Gaus");
        ...
        if self.higgs_sample=="ggH600":
            ...
            self.fit_mlvj_model_single_MC(self.file_ggH,"_%s"%(self.higgs_sample),"_signal_region","CB_v1");  
            self.fit_mlvj_model_single_MC(self.file_vbfH,"_%s"%(self.vbfhiggs_sample),"_signal_region","CB_v1"); 
            #self.fit_mlvj_model_single_MC(self.file_ggH,"_%s"%(self.higgs_sample),"_signal_region","ArgusBW_v1");  
            #self.fit_mlvj_model_single_MC(self.file_vbfH,"_%s"%(self.vbfhiggs_sample),"_signal_region","ArgusBW_v1"); 
        else:
            self.fit_mlvj_model_single_MC(self.file_ggH,"_%s"%(self.higgs_sample),"_sb_lo","BW_v1");
    ...
    def fit_VV(self):
        ...
        #self.fit_mj_single_MC(self.file_VV_mc,"_VVmassup","2Voig");
        #self.fit_mj_single_MC(self.file_VV_mc,"_VVmassdn","2Voig");
    ...
    def fit_TTbar(self):
        ...
        #self.get_mj_and_mlvj_dataset(self.file_TTbar_Powheg_mc,"_TTbar_Powheg")# to get the shape of m_lvj
        ...
        #self.fit_mj_single_MC(self.file_TTbar_mc,"_TTbarmassup","ErfExpGaus_sp");
        #self.fit_mj_single_MC(self.file_TTbar_mc,"_TTbarmassdn","ErfExpGaus_sp");
        ...
        if self.MODEL_4_mlvj=="ErfPowExp_v1":
            ...
            self.fit_mlvj_model_single_MC(self.file_TTbar_matchDn_mc,"_TTbar_matchDn","_signal_region","ErfExp_v1");
        else:
            self.fit_mlvj_model_single_MC(self.file_TTbar_mc,"_TTbar","_sb_lo",self.MODEL_4_mlvj);
        ...
        #self.fit_mlvj_model_single_MC(self.file_TTbar_Powheg_mc,"_TTbar_Powheg","_signal_region",self.MODEL_4_mlvj);
        self.fit_mlvj_model_single_MC(self.file_TTbar_MG_mc,"_TTbar_MG","_signal_region",self.MODEL_4_mlvj);
        if self.MODEL_4_mlvj=="ErfPowExp_v1" and self.mj_signal_min==65:
            self.fit_mlvj_model_single_MC(self.file_TTbar_mc,"_TTbar","_signal_region","ErfPowExp_v1_0",1);
        else:
    ...
    def fit_STop(self):
        ...
        self.fit_mj_single_MC(self.file_STop_mc,"_STopmassup","ErfExpGaus_sp");
        self.fit_mj_single_MC(self.file_STop_mc,"_STopmassdn","ErfExpGaus_sp");
...
class doFit_wj_and_wlvj_simultaneous:
    def __init__(self):
        ...
        print "Pure W-tagger SF of el     : %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_sf_el, pure_wtagger_sf_el_err, pure_wtagger_sf_el_err/pure_wtagger_sf_el)
        print "Pure W-tagger SF of mu     : %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_sf_mu, pure_wtagger_sf_mu_err, pure_wtagger_sf_mu_err/pure_wtagger_sf_mu)
        print "Pure W-tagger mean shift   : %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_mean_shift, pure_wtagger_mean_shift_err, pure_wtagger_mean_shift_err/pure_wtagger_mean_shift)
        print "Pure W-tagger sigma enlarge: %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_sigma_enlarge, pure_wtagger_sigma_enlarge_err, pure_wtagger_sigma_enlarge_err/pure_wtagger_sigma_enlarge)

        self.boostedW_fitter_el.file_out_ttbar_control.write( "\n***************************************************" )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger SF of el     : %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_sf_el, pure_wtagger_sf_el_err, pure_wtagger_sf_el_err/pure_wtagger_sf_el) )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger SF of mu     : %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_sf_mu, pure_wtagger_sf_mu_err, pure_wtagger_sf_mu_err/pure_wtagger_sf_mu) )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger mean shift   : %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_mean_shift, pure_wtagger_mean_shift_err, pure_wtagger_mean_shift_err/pure_wtagger_mean_shift) )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger sigma enlarge: %0.3f +/- %0.3f,  %0.3f"%(pure_wtagger_sigma_enlarge, pure_wtagger_sigma_enlarge_err, pure_wtagger_sigma_enlarge_err/pure_wtagger_sigma_enlarge) )
...
def pre_limit_sb_correction_without_systermatic( channel, higgs_sample="ggH600", in_mlvj_signal_region_min=500, in_mlvj_signal_region_max=700, in_mj_min=30, in_mj_max=140, in_mlvj_min=400, in_mlvj_max=1400, fit_model="ErfExp_v1", fit_model_alter="ErfPow_v1"): # the WJets M_lvj shape and normalization are from sb_correction
    boostedW_fitter=doFit_wj_and_wlvj(channel, higgs_sample, in_mlvj_signal_region_min, in_mlvj_signal_region_max, in_mj_min, in_mj_max, in_mlvj_min, in_mlvj_max,fit_model, fit_model_alter);
    boostedW_fitter.analysis_sideband_correction_method1_without_shape_and_psmodel_systermatic();


def pre_limit_sb_correction(method, channel, higgs_sample="ggH600", in_mlvj_signal_region_min=500, in_mlvj_signal_region_max=700, in_mj_min=30, in_mj_max=140, in_mlvj_min=400, in_mlvj_max=1400, fit_model="ErfExp_v1", fit_model_alter="ErfPow_v1"): # the WJets M_lvj shape and normalization are from sb_correction
...
def control_single_sb_correction(method, channel, higgs_sample="ggH600", in_mlvj_signal_region_min=500, in_mlvj_signal_region_max=700, in_mj_min=30, in_mj_max=140, in_mlvj_min=400, in_mlvj_max=1400, fit_model="ErfExp_v1", fit_model_alter="ErfPow_v1"): # the WJets M_lvj shape and normalization are from sb_correction
    print "pre_limit_sb_correction with %s for %s sample"%(method, channel)
    boostedW_fitter=doFit_wj_and_wlvj(channel, higgs_sample, in_mlvj_signal_region_min, in_mlvj_signal_region_max, in_mj_min, in_mj_max, in_mlvj_min, in_mlvj_max,fit_model, fit_model_alter);
    boostedW_fitter.ControlPlots();
...
def pre_limit_simple(channel):
    ...
    #pre_limit_sb_correction_without_systermatic(channel, "ggH600",550, 700,40,130,400,1000,"ErfExp_v1","ErfPow_v1");
