...
parser.add_option('-i', '--input', action='store', type='string', dest='origin', default='/uscms_data/d3/xyan/WTagging/Final_23')
...
#        return True
    ...
    if isMC:
        ...
        runTree.Draw("genEventCount_>>genH_%s" % sample, "", "goff")
    ...
    for event in range(0, tree.GetEntries()):
        if (event%10000==0 or event==nev-1): print ' = TTree:', tree.GetName(), 'events:', nev, '\t', int(100*float(event+1)/float(nev)), '%\r',
