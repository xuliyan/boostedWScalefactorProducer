...
def MakeGeneralPdf(workspace,label,model,spectrum,wtagger_label, channel,constraint,ismc=1):
	...
	if model == "ErfExp_ttbar" or model == "ErfExp_ttbar_failtau2tau1cut":
		...
		if model.find("failtau2tau1cut")!=-1:
			c0_tmp = -3.0626e-02    ; c0_tmp_err = 1.46e-02;
			offset_tmp = 5.1636e+01 ; offset_tmp_err = 4.92;
			width_tmp = 3.6186e+01  ; width_tmp_err = 4.69e+00;

			
			rrv_offset_ErfExp = RooRealVar("rrv_offset_ErfExp"+label+"_"+channel+spectrum ,"rrv_offset_ErfExp"+label+"_"+channel+spectrum ,offset_tmp, offset_tmp-offset_tmp_err,offset_tmp+offset_tmp_err) #these govern low-mass tail of "fail" fit. Can be fixed to avoid downwoards slope towards zero
			...
			c0_tmp     =  -3.3239e-02 ; c0_tmp_err     = 4.52e-03;
			offset_tmp =  9.6238e+01  ; offset_tmp_err = 3.53e+00;
			width_tmp  =  3.5444e+01  ; width_tmp_err  = 1.84e+00;
	...
	if model == "GausErfExp_ttbar" or model =="GausErfExp_ttbar_failtau2tau1cut":
		...
		frac_tmp = 1.0; #Use only Gaussian component to model real W-jets, ErfExp set to zero
    
		if model.find("fail")!=-1:
			c0_tmp     = -3.0626e-02 ; c0_tmp_err = 1.46e-02;
			offset_tmp = 5.1636e+01  ; offset_tmp_err = 4.92e+01;
			width_tmp  = 3.6186e+01  ; width_tmp_err = 14.69;
			...
			rrv_offset_ErfExp = RooRealVar("rrv_offset_ErfExp"+label+"_"+channel+spectrum ,"rrv_offset_ErfExp"+label+"_"+channel+spectrum ,offset_tmp,offset_tmp-offset_tmp_err,offset_tmp+offset_tmp_err)
			rrv_width_ErfExp  = RooRealVar("rrv_width_ErfExp" +label+"_"+channel+spectrum ,"rrv_width_ErfExp" +label+"_"+channel+spectrum ,width_tmp, width_tmp-width_tmp_err, width_tmp+width_tmp_err)
			...
			offset_tmp =  8.6888e+01 ; offset_tmp_err = 9.35e+00;
	...
	if model == "ErfExpGaus_sp":
		...
		rrv_sigma1_gaus  = RooRealVar("rrv_sigma1_gaus"+label+"_" +channel+spectrum,"rrv_sigma1_gaus" +label+"_"+channel+spectrum,7.,7.,40)
...
def makeTTbarModel(workspace,label, model,channel, wtagger, constraint=[], spectrum="_mj"):
	 ...
	 if label.find("_ttbar_data")!=-1 and label.find("fail")==-1:
		 ...
		 eff_ttbar = RooRealVar("eff_ttbar_data"+info+"_"+channel,"eff_ttbar_data"+info+"_"+channel,0.7,0.4,0.9)
		 ...
		 eff_ttbar = RooRealVar("eff_ttbar_TotalMC"+info+"_"+channel,"eff_ttbar_TotalMC"+info+"_"+channel,0.7,0.4,0.9)
		 rrv_number = RooFormulaVar("rrv_number"+label+"_"+channel+spectrum, "@0*@1", RooArgList(eff_ttbar,rrv_number_total))
