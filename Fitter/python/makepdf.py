...
def MakeGeneralPdf(workspace,label,model,spectrum,wtagger_label, channel,constraint,ismc=1):
	...
	if model == "ErfExp_ttbar" or model == "ErfExp_ttbar_failtau2tau1cut" or model == "ErfExp_ttbar_failtau2tau1cut_fitMC":
		
		if model.find("failtau2tau1cut")!=-1:
			c0_tmp = -2.6259e-02    ; c0_tmp_err = 1.46e-02;
			offset_tmp = 5.1220e+01 ; offset_tmp_err = 4.92;
...
def makeTTbarModel(workspace,label, model,channel, wtagger, constraint=[], spectrum="_mj"):
	 ...
	 if label.find("_ttbar_data")!=-1 and label.find("fail")==-1:
		 ...
		 eff_ttbar = RooRealVar("eff_ttbar_data"+info+"_"+channel,"eff_ttbar_data"+info+"_"+channel,0.7,0.4,1.0)
		 ...
		 eff_ttbar = RooRealVar("eff_ttbar_TotalMC"+info+"_"+channel,"eff_ttbar_TotalMC"+info+"_"+channel,0.7,0.4,1.0)
		 rrv_number = RooFormulaVar("rrv_number"+label+"_"+channel+spectrum, "@0*@1", RooArgList(eff_ttbar,rrv_number_total))
