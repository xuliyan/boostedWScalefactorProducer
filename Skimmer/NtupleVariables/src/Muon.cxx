...
if(  ((ana->detailLevel & Ntuple::MuonBoostedIsolation) == Ntuple::MuonBoostedIsolation)  ) {
     if (ana->m_connectsucceeded[26]) m_pfRhoCorrRelIso03Boost = &((*ana->pfRhoCorrRelIso03Boost)[idx]); else m_pfRhoCorrRelIso03Boost = 0; 
    if (ana->m_connectsucceeded[27]) m_pfRhoCorrRelIso04Boost = &((*ana->pfRhoCorrRelIso04Boost)[idx]); else m_pfRhoCorrRelIso04Boost = 0; 
    if (ana->m_connectsucceeded[28]) m_pfDeltaCorrRelIsoBoost = &((*ana->pfDeltaCorrRelIsoBoost)[idx]); else m_pfDeltaCorrRelIsoBoost = 0; 
    if (ana->m_connectsucceeded[29]) m_pfRelIsoBoost = &((*ana->pfRelIsoBoost)[idx]); else m_pfRelIsoBoost = 0; 
    if (ana->m_connectsucceeded[30]) m_photonIsoBoost = &((*ana->photonIsoBoost)[idx]); else m_photonIsoBoost = 0; 
    if (ana->m_connectsucceeded[31]) m_neutralHadIsoBoost = &((*ana->neutralHadIsoBoost)[idx]); else m_neutralHadIsoBoost = 0; 
    if (ana->m_connectsucceeded[32]) m_chargedHadIsoBoost = &((*ana->chargedHadIsoBoost)[idx]); else m_chargedHadIsoBoost = 0; 
    if (ana->m_connectsucceeded[33]) m_SemileptonicPFIso = &((*ana->SemileptonicPFIso)[idx]); else m_SemileptonicPFIso = 0; 
    if (ana->m_connectsucceeded[34]) m_SemileptonicCorrPFIso = &((*ana->SemileptonicCorrPFIso)[idx]); else m_SemileptonicCorrPFIso = 0; 
...
if(  ((ana->detailLevel & Ntuple::MuonID) == Ntuple::MuonID)  ) {
    ...
    if (ana->m_connectsucceeded[6]) m_isPFMuon = &((*ana->isPFMuon)[idx]); else m_isPFMuon = 0; 
    if (ana->m_connectsucceeded[7]) m_isGlobalMuon = &((*ana->isGlobalMuon)[idx]); else m_isGlobalMuon = 0; 
    if (ana->m_connectsucceeded[8]) m_isSoftMuon = &((*ana->isSoftMuon)[idx]); else m_isSoftMuon = 0; 
} // end of detail level ID

if(  ((ana->detailLevel & Ntuple::MuonIsolation) == Ntuple::MuonIsolation)  ) {
     if (ana->m_connectsucceeded[9]) m_pfRhoCorrRelIso03 = &((*ana->pfRhoCorrRelIso03)[idx]); else m_pfRhoCorrRelIso03 = 0; 
    if (ana->m_connectsucceeded[10]) m_pfRhoCorrRelIso04 = &((*ana->pfRhoCorrRelIso04)[idx]); else m_pfRhoCorrRelIso04 = 0; 
    if (ana->m_connectsucceeded[11]) m_pfDeltaCorrRelIso = &((*ana->pfDeltaCorrRelIso)[idx]); else m_pfDeltaCorrRelIso = 0; 
    if (ana->m_connectsucceeded[12]) m_pfRelIso = &((*ana->pfRelIso)[idx]); else m_pfRelIso = 0; 
...
if(  ((ana->detailLevel & Ntuple::MuonTrack) == Ntuple::MuonTrack)  ) {
    ...
    if (ana->m_connectsucceeded[19]) m_bestTrack_pt = &((*ana->bestTrack_pt)[idx]); else m_bestTrack_pt = 0; 
    if (ana->m_connectsucceeded[20]) m_bestTrack_ptErr = &((*ana->bestTrack_ptErr)[idx]); else m_bestTrack_ptErr = 0; 
    if (ana->m_connectsucceeded[21]) m_trackerHits = &((*ana->trackerHits)[idx]); else m_trackerHits = 0; 
    if (ana->m_connectsucceeded[22]) m_matchedStations = &((*ana->matchedStations)[idx]); else m_matchedStations = 0; 
    if (ana->m_connectsucceeded[23]) m_pixelHits = &((*ana->pixelHits)[idx]); else m_pixelHits = 0; 
    if (ana->m_connectsucceeded[24]) m_globalHits = &((*ana->globalHits)[idx]); else m_globalHits = 0; 
    if (ana->m_connectsucceeded[25]) m_normChi2 = &((*ana->normChi2)[idx]); else m_normChi2 = 0; 
}
