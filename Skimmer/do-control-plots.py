...
dir = "/scratch/thaarres/NANO_05feb/"
cutL = "(abs(dr_LepJet)>1.5708&&abs(dphi_MetJet)>2.&&abs(dphi_WJet)>2.&&Jet_btagCSVV2>0.8484&&FatJet_pt[0]>200&&FatJet_softDrop_mass>30)"
vars = ["FatJet_msoftdrop[0]"] #Pileup_nPU
bkgs = []
...
bkgs.append(WJs)
bkgs.append(TTs)
legs=["Single Top (2016)","VV (2017)","QCD (2017)","W+jets (2016)", "TT (2016)"]
genEvs = getGenEvents()
...
def doCP(postfix=""):
  for var in vars:
    ...
    bins = 36
    unit = "GeV"
    if var.find("softdrop")!=-1 or var.find("mass")!=-1: minx=0.; maxx=200.; bins=40;
    ...
    datahist = drawTH1("data",treeD,var,cutL,bins,minx,maxx,1,var.replace("_", " ").replace("[0]", "").replace("FatJet", "AK8 jet"),unit,"HIST","1")
